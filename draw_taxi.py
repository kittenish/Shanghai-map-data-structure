...
taxifind = find_taxi.taxi_information(taxi,idnum,node,way,relation,node1,graph,idx,lonlatid)
#nodetemp=node
#taxi,idnum,node,way,relation,node1,graph,idx,lonlatid=evaluate_taxi.evaluate_taxi()

#hits = idx.intersection((34, 25, 35, 28), objects=True)
class Map(QtGui.QWidget):
    ...
    def drawLines(self,qp):
        ...
        for i in way :
            ...
            pen7 = QtGui.QPen(QtCore.Qt.green,1.5,QtCore.Qt.SolidLine)
            pen8 = QtGui.QPen(QtCore.Qt.darkGreen,1.5,QtCore.Qt.SolidLine)
            for i in xrange(0,len(taxi)-1) :
                lon1=(float(taxifind[i].lon)-self.l)/self.maxt*size.height() + self.fx
                lon2=(float(taxifind[i+1].lon)-self.l)/self.maxt*size.height() + self.fx
                lat1=self.height()-(float(taxifind[i].lat)-self.b)/self.maxt*size.height() - self.fy
                lat2=self.height()-(float(taxifind[i+1].lat)-self.b)/self.maxt*size.height() - self.fy
                if taxifind[i].status == "1" :
                    qp.setPen(pen7)
                else :
                    qp.setPen(pen8)
                qp.drawLines(QtCore.QPoint(lon1,lat1),QtCore.QPoint(lon2,lat2))
