...
wayinfor = way_information.way_information(node,way,relation,node1,graph,idx)
class Map(QtGui.QWidget):
    ...
    def drawLines(self,qp):
        pen7 = QtGui.QPen(QtCore.Qt.darkYellow,2,QtCore.Qt.SolidLine)
        ...
        for i in range(0,des) :
            qp.setPen(pen7)
            des1 = len(wayinfor[i])
            for j in range(1,des1):
                lon1=(node[node1.get(wayinfor[i][j])].lon-self.l)/self.maxt*size.height() + self.fx
                lon2=(node[node1.get(wayinfor[i][j-1])].lon-self.l)/self.maxt*size.height() + self.fx
                lat1=self.height()-(node[node1.get(wayinfor[i][j])].lat-self.b)/self.maxt*size.width() - self.fy
                lat2=self.height()-(node[node1.get(wayinfor[i][j-1])].lat-self.b)/self.maxt*size.width() - self.fy
                qp.drawLines(QtCore.QPoint(lon1,lat1),QtCore.QPoint(lon2,lat2))
